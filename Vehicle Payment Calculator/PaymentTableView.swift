...
//                    Text("+ ").foregroundColor(.red) + Text("Base Total: \(formatCurrency(baseTotal))
                    ...
                    Text("(Total Amount Financed: \(formatCurrency(baseTotal - tradeIn - payment.cashDown)))").foregroundStyle(.secondary)
//                    Spacer()
                    Text("• Tax •")
                        .frame(maxWidth: .infinity, alignment: .center).foregroundStyle(.secondary)
                    Text("× ").foregroundColor(.red)
                    + Text("State Tax Rate \(payment.taxRate == 6.875 ? "(MN)" : "(Custom)"): \(payment.taxRate, specifier: "%.2f")%")
                    Text("× ").foregroundColor(.red)
                    + Text("Finance Rate: ")
